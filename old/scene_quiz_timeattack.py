...
<QuizScreen>:
    ...
    FlexibleLabel:
        ...
        color: [0.6, 0.6, 1.0, 1]
        text: '{}/{}'.format(root.n_cleared, root.n_answered)
    ClockLabel:
        size_hint: 0.25, 0.1
...
class QuizScreen(Screen):
    ...
    def on_pre_enter(self):
        ...
        self.n_cleared = quizstate.n_cleared
        self.n_answered = quizstate.n_answered
        self.time = quizstate.time
        quiz = quizstate.quiz
        # choosing_area
        n_choices = len(quiz.choices)
        choosing_area = self.ids['choosing_area']
        sentence_area = self.ids['sentence_area']
        choosing_area.clear_widgets()
        if len(self._all_buttons) < n_choices:
            for i in range(n_choices - len(self._all_buttons)):
        ...
        # sentence_area
        sentence = '\n'.join(quiz.facts)
        if quizstate.show_all_statements_at_once:
...
class Manager(ScreenManager):
    ...
    def __init__(self, *, appglobals, **kwargs):
        ...
        self._quizstate = attrdict(
            ...
            n_adjectives=None,               # a parameter for generating quiz
            n_choices=None,                  # a parameter for generating quiz
            quiz=None,                         # quiz object (from 'quizgenerator')
            show_all_statements_at_once=None,  # show Question with Statements?
            n_clear_to_next_level=None,      # number of quizzes to go to next level
            n_cleared=None,                  # number of quizzes you cleared
            n_answered=None,                 # number of quizzes you answered
    ...
    def update_quiz(self):
        ...
        quizstate.quiz = quizgenerator.generate_quiz(
            choices=self._quiz_settings['choices'][:quizstate.n_choices],
            n_adjectives=quizstate.n_adjectives,
            language=language)

    def goto_result(self):
        quizstate = self._quizstate
        points = quizstate.n_cleared / quizstate.n_answered / (100 + quizstate.time) * 10000
    ...
    def on_pre_enter(self):
        ...
        self._num_clear_to_finish = self._quiz_settings['levels'][-1]['n_clear_to_next_level']
        quizstate.update(
            lang_font_tuples=[
                (key, value['font_name'],) for key, value in
                data.lang_settings.available_langs()
            ],
            level=0,
            n_cleared=0,
            n_answered=0,
    ...
    def next_quiz(self):
        quizstate = self._quizstate
        if quizstate.is_correct and quizstate.n_cleared == quizstate.n_clear_to_next_level:
            if quizstate.n_cleared == self._num_clear_to_finish:
                self.goto_result()
            else:
                quizstate.level += 1
